	  ...
	  while checking do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' and not ESX.PlayerData.dead then
		  if distance < 2.0 then
			  DrawText3D(x,y,z, '[~g~E~s~] Megvizsgálás', 0.4)
  ...
  function OpenDeathMenu(player)
	  ...
	  table.insert(elements, {label = 'Halál indok', value = 'deathcause'})
	  table.insert(elements, {label = 'Sérülés helye', value = 'damage'})
  
  
	  ESX.UI.Menu.Open(
		  'default', GetCurrentResourceName(), 'dead_citizen',
		  {
			  title    = 'Válassz valamit',
			  align    = 'right',
			  elements = elements,
		  },
	  function(data, menu)
		  ...
		  if ac == 'damage' then
			  ...
			  if success then
				  ...
				  Notify('Nem tudtad megállapítani a sérülés helyét (Lehet nincs)')
			  end
		  end
